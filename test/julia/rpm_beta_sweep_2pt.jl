...
# -----------------------------
# Method 1: Adaptive DC tracking (linear feedback)
# -----------------------------
function rpm_adaptive(omega, beta, N; alpha=0.001)
    y = zeros(Float64, N)
    dc = 0.0
    phase = 0.0
    for n in 2:N
...
# -----------------------------
# Method 2: Power-normalized (RMS² tracking, linear feedback)
# Scale by 0.5 to match squared feedback modulation depth
# (linear has amplitude 1.0 vs squared's 0.5 after DC removal)
# -----------------------------
function rpm_power_norm(omega, beta, N; alpha=0.001)
    y = zeros(Float64, N)
    ...
    for n in 2:N
        ...
        # normalize by RMS (amplitude), scale by 0.5 for comparable modulation depth
...
# -----------------------------
methods = [
    ("Adaptive DC", rpm_adaptive),
    ("Power-normalized", rpm_power_norm)
]
...
# Save and open
outfile = tempname() * "_rpm_dc_comparison.png"
savefig(final_plot, outfile)
run(`open $outfile`)

println("\nRPM Linear Feedback DC Compensation Comparison")
println("===============================================")
println("f0 = $f0 Hz")
println("β values: $betas")
println("Methods: Adaptive DC, Power-normalized")
println("")
println("Usage: julia rpm_beta_sweep_2pt.jl [f0]")
println("       julia rpm_beta_sweep_2pt.jl 440")
println("")
